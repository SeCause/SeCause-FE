const STEPS = [
  { label: 'Detect', detail: '취약점 탐지', className: 'logo-step-one' },
  { label: 'Explain', detail: '원인 분석', className: 'logo-step-two' },
  { label: 'Resolve', detail: '수정 제안', className: 'logo-step-three' },
] as const;
...
export default function LogoAssembly() {
  return (
    <div className="relative mx-auto aspect-square w-full max-w-110" aria-label="SeCause 분석 과정">
      ...
      <div className="logo-orbit border-blue/20 absolute inset-[3%] rounded-full border border-dashed" />
      ...
      <div className="absolute inset-[23%] flex items-center justify-center rounded-[30%] bg-[#f3f6fd] shadow-[0_25px_65px_rgba(53,109,230,0.13)]">
        <div className="logo-scan-line" aria-hidden="true" />
        <div className="flex flex-col items-center">
          ...
          >
            <g fill="#356de6" className="logo-pieces logo-mark">
              <g className="logo-piece">
                <rect
                  x="18.1292"
                  y="17.9541"
                  width="2.04676"
          ...
          <div className="relative mt-1 flex h-8 items-center justify-center md:h-9">
            <p
              aria-hidden="true"
              className="logo-codemark text-blue absolute font-mono text-xl font-semibold tracking-[0.12em] md:text-2xl"
            >
              &lt;/&gt;
            </p>
            <p className="logo-wordmark absolute text-2xl font-semibold tracking-[-0.055em] text-gray-900 md:text-3xl">
              SeCause
            </p>
          </div>
        </div>
      ...
      {STEPS.map((step, index) => (
        <div
          key={step.label}
          className={`logo-analysis-step border-blue/15 absolute rounded-full border bg-white px-4 py-2.5 shadow-[0_12px_35px_rgba(53,109,230,0.1)] ${step.className}`}
        >
